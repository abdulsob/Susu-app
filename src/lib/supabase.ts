...
export async function recordDeposit(
  ...
  // Convert local amount to USDC
  const usdcAmount = amount / exchangeRate;

  // Update both local balance and USDC balance
  await supabase
    .from("users")
    .update({
      balance: (user.balance ?? 0) + amount,
      usdc_balance: (user.usdc_balance ?? 0) + usdcAmount,
    })
    .eq("privy_id", privyId);
...
export async function saveWalletInfo(privyId: string, walletId: string, walletAddress: string) {
  await supabase
    .from("users")
...
export async function getWalletInfo(privyId: string) {
  const { data } = await supabase
    .from("users")
    .select("wallet_id, wallet_address, usdc_balance")
    .eq("privy_id", privyId)
    .single();
  return data;
}

export async function updateUsdcBalance(privyId: string, usdcBalance: number) {
  await supabase
    .from("users")
...
export async function enableCryptoMode(privyId: string, walletId: string, walletAddress: string) {
  await supabase
    .from("users")
    .update({
      crypto_mode: true,
...
export async function getCryptoMode(privyId: string): Promise<boolean> {
  const { data } = await supabase
    .from("users")
    .select("crypto_mode")
    .eq("privy_id", privyId)
    .single();
  return data?.crypto_mode ?? false;
}
