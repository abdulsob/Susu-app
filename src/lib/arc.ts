...
// ─── Get USDC balance by wallet address (Arc testnet) ────────────────────────

export async function getWalletBalanceByAddress(address: string): Promise<number> {
  try {
    // Arc testnet uses Circle's blockchain API
    const res = await fetch(
      `https://api.circle.com/v1/w3s/wallets?address=${address}&blockchain=ARB-SEPOLIA`,
      {
        headers: {
          Authorization: `Bearer ${CIRCLE_API_KEY}`,
          "Content-Type": "application/json",
    ...
    console.warn("Arc balance fetch failed:", e);
    return 0;
  }
}
